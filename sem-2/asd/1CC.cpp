...
struct segtree {
    ...
    void propogate(int x) {
        if (tree[x].last) {
            tree[2 * x + 1].m = tree[x].set;
            tree[2 * x + 1].set = tree[x].set;
            tree[2 * x + 1].last = true;
            tree[2 * x + 2].m = tree[x].set;
            tree[2 * x + 2].set = tree[x].set;
            tree[2 * x + 2].last = true;
        } else {
            tree[2 * x + 1].m += tree[x].add;
            tree[2 * x + 1].add = tree[x].add;
            tree[2 * x + 1].last = false;
            tree[2 * x + 2].m += tree[x].add;
            tree[2 * x + 2].add = tree[x].add;
            tree[2 * x + 2].last = false;
        }
        tree[x].add = 0;
