...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    override fun deposit(index: Int, amount: Long): Long { // First, validate method per-conditions
        ...
        while (true) {
            ...
            if (checkReturnState(index, account, updated)) return updated.amount
        }
    }
    ...
    private fun acquire(index: Int, op: Op): AcquiredAccount? {
        ...
        while (true) {
            ...
            if (checkReturnState(index, account, acquiredAccount)) return acquiredAccount
    ...
    private fun invoke(account: Account): Boolean {
        return account.invokeOperation()
    }

    private fun checkReturnState(index: Int, account: Account, updated: Account): Boolean {
        return accounts[index].compareAndSet(account, updated)
    }
